...
export function Newsletter() {
  ...
  return (
    <section className="px-5 pb-24 sm:px-8 sm:pb-32">
      <Reveal mode="blur" className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-primary/10 bg-gradient-to-br from-soft-blush-700 via-soft-blush-900 to-honeydew-700 px-6 py-14 text-center shadow-soft sm:rounded-[2.5rem] sm:px-12 sm:py-20">
        ...
        <div className="relative mx-auto max-w-2xl">
          ...
          <form onSubmit={(event) => { event.preventDefault(); setSent(true); }} className="mx-auto mt-9 flex max-w-xl flex-col gap-3 rounded-2xl border border-border bg-soft-blush-900 p-2 shadow-soft sm:flex-row">
          ...
          <AnimatePresence>{sent && <motion.p role="status" initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="mt-4 text-sm font-semibold text-india-green-500">شكراً لك! أضفناك إلى قائمتنا بنجاح.</motion.p>}</AnimatePresence>
        </div>
      </Reveal>
    </section>
  );
