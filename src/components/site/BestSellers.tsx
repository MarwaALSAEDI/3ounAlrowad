...
export function BestSellers() {
  return (
    <section id="best" className="relative overflow-hidden bg-soft-blush-900 py-24 sm:py-32">
...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <motion.article variants={itemVariants} whileHover={{ y: -10 }} transition={{ type: "spring", stiffness: 220, damping: 24 }} className="group relative overflow-hidden rounded-[1.75rem] border border-border bg-card p-4 shadow-soft transition-shadow duration-500 hover:shadow-lift sm:p-5">
      <div className="relative aspect-[4/3] overflow-hidden rounded-3xl bg-gradient-to-br from-soft-blush-800 to-honeydew-800">
        ...
        <motion.button type="button" onClick={() => setLiked((value) => !value)} whileTap={{ scale: 0.84 }} aria-label={liked ? "إزالة من المفضلة" : "إضافة إلى المفضلة"} aria-pressed={liked} className="absolute left-3 top-3 z-10 grid h-10 w-10 place-items-center rounded-xl border border-soft-blush-900/70 bg-soft-blush-900/85 shadow-soft backdrop-blur transition-colors hover:text-primary">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-soft-blush-900/55 to-transparent" />
      </div>

      <div className="mt-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-1 text-brick-ember-500">
