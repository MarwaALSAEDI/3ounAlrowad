    ...
    <section id="offer" ref={ref} className="px-5 py-20 sm:px-8 sm:py-28">
      <div className="dark relative mx-auto grid max-w-7xl overflow-hidden rounded-[2rem] bg-background px-6 py-12 text-foreground shadow-lift sm:rounded-[2.5rem] sm:px-12 sm:py-16 lg:grid-cols-2 lg:items-center lg:gap-12 lg:px-16">
        ...
        <motion.div initial={{ opacity: 0, y: 35 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.35 }} transition={{ duration: 0.8 }} className="relative z-10">
          <span className="eyebrow rounded-full border border-white/10 bg-white/5 px-4 py-2"><Timer className="h-4 w-4" /> عرض الموسم لفترة محدودة</span>
          ...
          <div className="mt-6 flex flex-wrap gap-4 text-xs font-semibold text-white/75 sm:text-sm">
          ...
          <motion.a href="#best" whileHover={{ scale: 1.035, boxShadow: "0 20px 50px -18px rgb(34 197 94 / .55)" }} whileTap={{ scale: 0.97 }} className="group mt-9 inline-flex min-h-14 items-center gap-3 rounded-2xl bg-accent px-8 font-bold text-accent-foreground">
        ...
        <div className="relative z-10 mt-14 h-[320px] lg:mt-0 lg:h-[430px]">
          ...
          <motion.img src={airfryer} alt="قلاية هوائية ضمن العرض" loading="lazy" decoding="async" width={1024} height={1024} style={{ y: productYSlow }} className="absolute right-0 top-0 w-[58%] drop-shadow-2xl" />
          <motion.img src={cookware} alt="طقم أواني ضمن العرض" loading="lazy" decoding="async" width={1024} height={1024} style={{ y: productY }} className="absolute bottom-0 left-0 w-[59%] drop-shadow-2xl" />
          <motion.img src={kettle} alt="غلاية كهربائية ضمن العرض" loading="lazy" decoding="async" width={1024} height={1024} animate={{ y: [0, -12, 0] }} transition={{ duration: 5.5, repeat: Infinity, ease: "easeInOut" }} className="absolute left-[33%] top-[7%] w-[30%] drop-shadow-2xl" />
